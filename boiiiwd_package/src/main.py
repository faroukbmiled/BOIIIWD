...
class BOIIIWD(ctk.CTk):
    def __init__(self):
        ...
        # Set window icon
        png_path = os.path.join(RESOURCES_DIR, "ryuk.png")
        ...
        # Don't save geometry on every configure event - only save on close to avoid race conditions
    ...
    def on_closing(self):
        print(f"[{get_current_datetime()}] [Logs] on_closing invoked, saving settings...")

        # Save window geometry first
        self.save_window_size_position()
        ...
        print(f"[{get_current_datetime()}] [Logs] Settings saved, stopping downloads...")
        self.stop_download(on_close=True)

        print(f"[{get_current_datetime()}] [Logs] Exiting...")
        self.destroy()
        sys.exit(0)
    ...
    def show_steam_warning_message(self):
        def callback():
            msg = CTkMessagebox(master=self, title="Warning", message=f"{STEAMCMD_EXE} was not found in the specified directory.\nPress Download to get it or Cancel to select it manually.",
    ...
    def download_steamcmd(self):
        ...
        steamcmd_archive_path = os.path.join(STEAMCMD_DIR, "steamcmd.tar.gz")

        try:
            response = requests.get(steamcmd_url)
            response.raise_for_status()

            with open(steamcmd_archive_path, "wb") as archive_file:
                archive_file.write(response.content)

            import tarfile
            with tarfile.open(steamcmd_archive_path, "r:gz") as tar_ref:
            ...
            if check_steamcmd():
                os.remove(steamcmd_archive_path)
                def inti_steam():
                    msg = CTkMessagebox(master=self, title="Success", message="SteamCMD has been downloaded.\n\nIMPORTANT: You need 32-bit libraries installed!\nRun: sudo apt install lib32gcc-s1\n\nPress Ok to initialize SteamCMD.", icon="info", option_1="No", option_2="Ok", sound=True,
                ...
                show_message("Error", f"Failed to find {STEAMCMD_EXE} after extraction.\nMake sure to select the correct SteamCMD path", icon="cancel")
                os.remove(steamcmd_archive_path)
        except requests.exceptions.RequestException as e:
            show_message("Error", f"Failed to download SteamCMD: {e}", icon="cancel")
            try: os.remove(steamcmd_archive_path)
            except: pass
        except Exception as e:
            show_message("Error", f"Failed to extract SteamCMD. The downloaded file might be corrupted.\n{e}", icon="cancel")
            try: os.remove(steamcmd_archive_path)
            except: pass
                ...
                def main_thread():
                    ...
                    if os.path.exists(icon_path):
                        try:
                            from PIL import Image, ImageTk
                            icon_image = ImageTk.PhotoImage(Image.open(icon_path))
                            description_window.after(210, lambda: description_window.iconphoto(False, icon_image))
                            description_window._icon_image = icon_image  # Keep reference
                        except Exception:
                            pass
    ...
    def check_steamcmd_stdout(self, log_file_path, target_item_id):
        ...
        if not hasattr(self, '_log_check_count'):
            self._log_check_count = 0
        self._log_check_count += 1

        if not os.path.exists(log_file_path):
            if self._log_check_count % 10 == 1:  # Log every 10 checks
                print(f"[{get_current_datetime()}] [Logs] workshop_log not found at: {log_file_path}")
            if os.path.isdir(log_file_path):
                try: os.makedirs(log_file_path)
                except: return False
            else:
        ...
        try: shutil.copy2(log_file_path, temp_file_path)
            ...
            if self._log_check_count % 10 == 1:
                print(f"[{get_current_datetime()}] [Logs] Failed to copy log file: {e}")
            return False
        ...
        try:
            with open(temp_file_path, 'r', errors="ignore") as log_file:
                ...
                if self._log_check_count % 10 == 1:
                    print(f"[{get_current_datetime()}] [Logs] workshop_log size: {file_size} bytes")
                ...
                # Log the lines we're checking
                if self._log_check_count % 10 == 1 and lines:
                    print(f"[{get_current_datetime()}] [Logs] Last {len(lines)} lines in workshop_log:")
                    for line in lines:
                ...
                for line in reversed(lines):
                    ...
                    if f"download item {target_item_id.strip()}" in line:
                        print(f"[{get_current_datetime()}] [Logs] Found download start for {target_item_id}!")
                        return True

            return False
        except Exception as e:
            print(f"[{get_current_datetime()}] [Logs] Error checking stdout: {e}")
            try:
                os.remove(temp_file_path)
            except:
                return False
    ...
    def check_download_started_from_output(self, buff, target_item_id):
        """Check if SteamCMD output indicates download has started."""
        if not buff:
            return False

        # Handle multi-line output - check each line
        buff_lower = buff.lower()
        target_id = str(target_item_id).strip()

        # Check for various download start indicators
        indicators = [
            f"download item {target_id}",
            f"downloading item {target_id}",
            f"workshop download for item {target_id}",
            "downloading workshop",
            "downloading item",  # Generic match without ID
            "success. downloaded item"
        ]

        for indicator in indicators:
            if indicator in buff_lower:
                print(f"[{get_current_datetime()}] [Logs] Detected download start from PTY output: '{indicator}'")
                return True
    ...
    def process_output_reader(self, process: PtyProcess):
        try:
    ...
    def run_steamcmd_command(self, command, map_folder, wsid, queue=None):
        ...
        steam_logs_dir = os.path.join(steam_base, "logs")

        # On Linux, SteamCMD uses ~/Steam/logs/ for all logs
        steamcmd_bootstrap_logs = os.path.join(steam_logs_dir, "bootstrap_log.txt")
        stdout_path = os.path.join(steam_logs_dir, "workshop_log.txt")

        timestamp = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        using_creds = self.settings_tab.use_steam_creds_sw.get()
        console = False  # Not used on Linux
        elapsed_time = 0
        steamcmd_restared_counter = -1 # So first run dont count

        # Get download folder path for progress tracking
        download_folder = get_workshop_downloads_folder(wsid)

        print(f"[{get_current_datetime()}] [Logs] Steam base path: {steam_base}")
        print(f"[{get_current_datetime()}] [Logs] Steam logs dir: {steam_logs_dir}")
        print(f"[{get_current_datetime()}] [Logs] workshop_log path: {stdout_path}")
        print(f"[{get_current_datetime()}] [Logs] map_folder (expected): {map_folder}")
        print(f"[{get_current_datetime()}] [Logs] download_folder: {download_folder}")

        try: os.makedirs(steam_logs_dir, exist_ok=True)
        except: pass
        ...
        try:
            ...
            print(f"[{get_current_datetime()}] [Logs] Could not clear log files: {e}")
            try: os.rename(stdout_path, os.path.join(map_folder, f"workshop_log_couldntremove_{timestamp}.txt"))
            except: pass

        # Clear content_log.txt for dump monitor to read fresh lines
        content_log_path = os.path.join(steam_logs_dir, "content_log.txt")
        try:
        ...
        if os.path.exists(map_folder):
            try:
                try:
                    ...
                    os.rename(map_folder, get_workshop_folder(f"couldntremove_{timestamp}"))
            except Exception as e:
                self.settings_tab.stopped = True
        ...
        if self.settings_tab.continuous:
            print(f"[{get_current_datetime()}] [Logs] Continuous mode enabled")
            if using_creds:
                if check_config("login_cached", "off") != "on":
                    ...
                    initiate_login_process(f'{steamcmd_exe} {command}', console)
            start_time = 0
            while not os.path.exists(map_folder) and not self.settings_tab.stopped:
                steamcmd_restared_counter += 1
                print(f'[{get_current_datetime()}] [Logs] attempting : {STEAMCMD_EXE} {command}')
                ...
                while True:
                    ...
                    if buff == "EOFError":
                        break
                    if buff:
                        print(f"[{get_current_datetime()}] [Logs] steamcmd output: {buff}")
                    if using_creds:
                    ...
                    if not self.is_downloading:
                        ...
                        # Check PTY output first (most reliable on Linux)
                        if self.check_download_started_from_output(buff, wsid):
                            print(f"[{get_current_datetime()}] [Logs] Download started (detected from PTY output)")
                            start_time = time.time()
                            self.is_downloading = True
                            self.is_steamcmd_updating = False
                        # Also check workshop_log.txt as fallback
                        elif self.check_steamcmd_stdout(stdout_path, wsid):
                            print(f"[{get_current_datetime()}] [Logs] Download started (detected from workshop_log)")
                            start_time = time.time()
                            self.is_downloading = True
                            self.is_steamcmd_updating = False
                        # Also check if files are appearing in the download folder
                        elif os.path.exists(download_folder) and os.listdir(download_folder):
                            print(f"[{get_current_datetime()}] [Logs] Download started (files detected in download folder)")
                            start_time = time.time()
                            self.is_downloading = True
                            self.is_steamcmd_updating = False
                ...
                print(f"[{get_current_datetime()}] [Logs] stopped={self.settings_tab.stopped}, continuous={self.settings_tab.continuous}, map_exists={os.path.exists(map_folder)}")
                if not self.settings_tab.stopped:
                    ...
                    if elapsed_time < SECONDS_UNTIL_FAIL_COUNTS and not os.path.exists(map_folder):
                        self.fail_threshold += 1
                        print(f"[{get_current_datetime()}] [Logs] fail_threshold incremented to {self.fail_threshold} (elapsed < {SECONDS_UNTIL_FAIL_COUNTS}s)")

                print(f"[{get_current_datetime()}] [Logs] steam_fail_counter_toggle={self.settings_tab.steam_fail_counter_toggle}")
                if self.settings_tab.steam_fail_counter_toggle:
                    try:
                        ...
                        if self.fail_threshold >= FAIL_THRESHOLD_FALLBACK:
                            reset_steamcmd(no_warn=True)
                            self.settings_tab.steamcmd_reset = True
                            self.settings_tab.steam_fail_counter = 0
                            self.fail_threshold = 0
                print(f"[{get_current_datetime()}] [Logs] Continuing outer while loop (retry)... stopped={self.settings_tab.stopped}")
                continue
            # After while loop exits
            print(f"[{get_current_datetime()}] [Logs] Continuous while loop exited: map_exists={os.path.exists(map_folder)}, stopped={self.settings_tab.stopped}")
        else:
            if using_creds:
                if check_config("login_cached", "off") != "on":
                    show_message("Please wait...", "A window will open shortly that will propmt you to login!, close it as soon as you're done with logging in!", icon="warning")
        ...
        # return process.returncode # OLD RETURN FROM SUBPROCESS
    ...
    def queue_download_thread(self, update=None):
        ...
        try:
            ...
            for index, item in enumerate(items):
                ...
                self.after(1, lambda mid=workshop_id: self.label_file_size.configure(text=f"File size: {get_workshop_file_size(mid ,raw=True)}"))
                download_folder = get_workshop_downloads_folder(workshop_id)
                map_folder = get_workshop_folder(workshop_id)
                    ...
                    while not self.settings_tab.stopped:
                        ...
                        if self.item_skipped:
                            if index > 0:
                                ...
                                prev_item_path = get_workshop_downloads_folder(previous_item)
                                prev_item_path_2 = get_workshop_folder(previous_item)
                ...
                def wait_for_threads():
                    ...
                    if os.path.exists(json_file_path):
                        ...
                        # Clean up SteamCMD data for this workshop ID (files already copied to destination)
                        cleanup_steamcmd_data(workshop_id, delete_content=True)
